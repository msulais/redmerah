...
type SideBarProps = astroHTML.JSX.HTMLAttributes & {
	...
	SideBarContainerAttr?: astroHTML.JSX.HTMLAttributes
...
function updateSideBarRef<T extends SideBarElement<HTMLElement>>(
	...
	// cotainer
	let containerRef = sideBarRef.querySelector<HTMLDivElement>(`.${SideBarClasses.container}`)
	if (!containerRef) {
		containerRef = document.createElement('div')
		containerRef.classList.add(SideBarClasses.container)
	}
	...
	containerRef.replaceChildren(...[headerRef, contentRef, footerRef].filter(
		v => typeof v === 'string' || v instanceof Node
	))
	sideBarRef.replaceChildren(containerRef)
