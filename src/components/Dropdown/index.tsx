...
import { array_concat, array_filter, array_find_index, array_includes, array_join, array_length, array_map, array_push, array_slice, array_some, array_splice } from "@/utils/array"
...
const DropdownOption: ParentComponent<DropdownOptionProps> = ($props) => {
	...
	const selected = createMemo<boolean>(() => {
		...
		return selected_values == undefined
			? false
			: array_includes(array_map(selected_values, v => `${v}`), `${props.value}`)
	})
	let value: string | number | null = null

	createEffect(() => {
		context?.on_cleanup_option(value) // remove old value
		context?.on_mount_option(props.value, props.text)
		value = props.value
	})

	onCleanup(() => context?.on_cleanup_option(props.value))
...
const Dropdown: ParentComponent<DropdownProps> = ($props) => {
	...
	const is_selected = createSelector<(string | number)[], string | number>(
		() => selected_values,
		(value, array) => array_includes(array, value)
	)
	...
	createEffect(() => {
		...
		if (values != undefined) set_selected_values(values)
	...
	return (<DropdownContext.Provider value={{
		on_mount_option: (value, text) => {
			...
			if (array_some($options, o => o.value == value)) return;
			set_options(d => [...d, {value, text}])
