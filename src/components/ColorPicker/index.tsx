import { type Component, type JSX, type ParentComponent, createEffect, createMemo, createSignal, mergeProps, onCleanup, onMount, splitProps } from "solid-js"
...
import { _body, _onCancel, _children, _disabledOpacityControl, _onToggle, _onSelectColor, _initialColor, _disabledColorControl, _ref, _onClose, _HEX, _round, _toString, _padStart, _toUpperCase, _hue, _position, _opacity, _HSL, _color, _RGB, _value, _parseFloat, _toFixed, _join, _substring, _isDrag, _max, _min, _rect, _left, _top, _touchmove, _touches, _clientX, _clientY, _touchend, _noPointerEvent, _mousemove, _mouseup, _length, _parseInt, _replace, _split, _push, _isNaN, _isFinite, _trim, _open, _observe, _disconnect, _currentTarget, _px } from "@/data/string"
...
type ColorPickerProps = JSX.DialogHtmlAttributes<HTMLDialogElement> & {
...
const ColorPicker: ParentComponent<ColorPickerProps> = ($props) => {
    ...
    const [initialHexColor, setInitialHexColor] = createSignal<string>('#FF0000')
    ...
    function updateInputs(onBeforeUpdate?: () => void): void {
        ...
        if (colorModel() == _RGB){
            ...
            colorInputRef[_value] = `${hexColor()[_substring](0, 7)}`
        }
    ...
    function initColor(): void {
        testHexColorWithAlpha(props[_initialColor])
        setHslColor(hexToHSL(props[_initialColor][_substring](0, 7)))

        if (props[_initialColor][_length] == 9 && !props[_disabledOpacityControl]) {
            const opacity = numberParse(props[_initialColor][_substring](7, 9), true, 16) / 255
            setOpacity(mathRound(opacity * 100))
        }

        if (props[_disabledColorControl]) setHslColor(hsl => {return {...hsl, s: 1, l: 0.5}})

        setInitialHexColor(props[_initialColor])
        updateInputs()
        updatePosition()
    }
    ...
    createEffect(() => {
        if (props[_initialColor] == initialHexColor()) return
        initColor()
    })
    ...
    const Actions: Component = () => {
        return (<div class="color-picker-actions">
            <Button onClick={changeColorModel} variant="filled-tonal">{colorModel()}</Button>
            <Button 
            ...
            <Button 
                variant="filled" 
                onClick={() => {
                    if (props[_onSelectColor]) props[_onSelectColor](hexColor())
                    closePopover(colorPickerRef)
                }}>
                Select
            </Button>
