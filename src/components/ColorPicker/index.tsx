...
function openPopoverColorPicker(
	...
	options?: Omit<PopoverOpenDetail, 'event'> & { color?: HEXColor }
): void {
	if (options?.[_color] != null) changePopoverColorPickerValue(colorPicker, options[_color])

	openPopover(event, colorPicker, options)
}

function changePopoverColorPickerValue(colorPicker: HTMLDivElement, color: HEXColor): void {
	colorPicker[_dispatchEvent](new CustomEvent(
		ColorPickerEvents[_onChangeColor],
		{ detail: color }
	))
