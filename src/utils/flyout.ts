...
import { closePopover, focusPopover, repositionPopover } from "./popover"
...
import { closeModal, focusModal } from "./modal"
import { _flyoutListener, _flyout, _open, _noPointerEvent, _clientX, _touches, _clientY, _focus, _x, _left, _right, _y, _top, _bottom, _modal, _popover, _activeElement, _flyoutOpen, _scrollY, _documentElement, _scrollTop, _scrollTo, _anchorId, _position, _observe, _click, _scroll, _resize, _join, _instant, _manual } from "@/data/string"
...
export function initFlyout(): void {
    ...
    // use for clicked outside flyout
    addEventListener(getDocument(), _click, async (ev: Event) => {
        if (hasAttribute(getDocumentBody(), BodyAttributes[_noPointerEvent])) return;
        ...
        const flyout: HTMLElement | HTMLDialogElement | null = querySelector(selector + ':focus')
        if (!flyout) return;
        ...
        if (getAttribute(flyout, 'data-dismiss') == _manual) {
            if (hasAttribute(flyout, ModalAttributes[_modal])) focusModal(flyout as HTMLDialogElement)
            else if (hasAttribute(flyout, PopoverAttributes[_popover])) focusPopover(flyout as HTMLElement)
            return
        }

        if (hasAttribute(flyout, ModalAttributes[_modal])) await closeModal(flyout as HTMLDialogElement)
        else if (hasAttribute(flyout, PopoverAttributes[_popover])) await closePopover(flyout as HTMLElement)

        if (!querySelector(selector)) removeAttribute(getDocumentBody(), BodyAttributes[_flyoutOpen])
    })
