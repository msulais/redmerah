 ...
 */
export function string_split(text: string, separator: string | RegExp, limit?: number): string[] {
 ...
 */
export function string_substring(text: string, start: number, end?: number): string {
...
export function string_totitlecase(text: string): string {
	return array_join(
		array_map(
			string_split(text, ' '),
			v => string_touppercase(string_substring(v, 0, 1)) + string_substring(string_tolowercase(v), 1)
	), ' ')
}
...
export function string_totogglecase(text: string): string {
	...
	for (const char of text) {
		...
		array_push(result, is_lower ? string_touppercase(char) : string_tolowercase(char))
	}
...
// type error (by astro check): Cannot find name 'RegExpStringIterator' from RegExpStringIterator<RegExpExecArray>
): RegExpMatchArray[] {
 ...
 * @param mex_length The length of the resulting string once the current string has been padded. If this parameter is smaller than the current string's length, the current string will be returned as it is.
