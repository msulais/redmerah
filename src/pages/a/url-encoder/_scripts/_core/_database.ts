...
import { DecoderStore, type DecoderStoreType } from "./_decoder"
...
type _StorageItems = {
	decode: DecoderStoreType['decode']
...
const _db = new IDB(DatabaseNames.URLEncoder)
...
function _readStorageAll(store: IDBObjectStore): void {
	_db.cursor(store, (cursor) => {
		...
		case "decode":
			isString
			&& DecoderStore.update(v => v.decode = value)
			break
